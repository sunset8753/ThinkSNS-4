...
include_once __DIR__.'/CacheTalk.php';
...
class DingTalk
{
    ...
    {
        $use_redis = CacheTalk::get(self::DINGTALK_CORP_ID.'_access_token');
        if ($refresh) {
            $use_redis = CacheTalk::set(self::DINGTALK_CORP_ID.'_access_token', null);
        }
        if ($use_redis) {
            $access_token = CacheTalk::get(self::DINGTALK_CORP_ID.'_access_token');
        ...
        if (!$use_redis) {
            $access_token = self::internal_get_access_token()->access_token;
                ...
                CacheTalk::set(self::DINGTALK_CORP_ID.'_access_token', (string) $access_token, 7140);
        }
    ...
    {
        $use_redis = CacheTalk::get(self::DINGTALK_CORP_ID.'_jsapi_ticket');
        if ($refresh) {
            $use_redis = CacheTalk::set(self::DINGTALK_CORP_ID.'_jsapi_ticket', null);
        }
        if ($use_redis) {
            $jsapi_ticket = CacheTalk::get(self::DINGTALK_CORP_ID.'_jsapi_ticket');
            $use_redis = $jsapi_ticket;
        }
        if (!$use_redis) {
            $jsapi_ticket_result = self::internal_get_jsapi_ticket();
            $jsapi_ticket = $jsapi_ticket_result->ticket;
            $expire_time = (int) $jsapi_ticket_result->expire_time - 60;
            if ($expire_time < 0) {
                $expire_time = 7140;
            }
            CacheTalk::set(self::DINGTALK_CORP_ID.'_jsapi_ticket', (string) $jsapi_ticket, $expire_time);
        }
