 ...
 * @param  bool   $autoRedirect 是否自动跳转，默认true(可选)
 * @param  bool   $outputError  是否输出错误消息，默认true(可选)
 * @return bool   是否校验通过
 */
function dingtalk_check_auth(&$token = null, $backUrl = null, $autoRedirect = true, $outputError = true)
{
    ...
    if (!$is_auth) {
        ...
        if ($autoRedirect) {
            redirect_302('/dingtalk/Core/auth.php?token='.$token);
        }
        if ($outputError) {
            die_error(USER_LOGIN_EXPIRED, 'Authorization failed, access denied.');
        }
