 ...
 * @return bool   是否请求成功
 */
function curl_http_request($url, $method = 0, $post_data = null, $cookie = null, $type = null)
{
    if (isset($post_data) && is_string($post_data) && strlen($post_data) > 0) {
        $post_fields = $post_data;
    } elseif (isset($post_data) && is_array($post_data) && count($post_data) > 0) {
