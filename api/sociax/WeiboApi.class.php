...
class WeiboApi extends Api
{
    ...
    public function public_timeline()
    {
        ...
        $list = $this->format_feed($feed_ids);
        if (!$list) {
            return array(
                'status' => 0,
                'msg' => '暂无更多',
    ...
    //热门微博
    public function hot_weibo()
    {
        $max_id = $this->data['max_id'] ? intval($this->data['max_id']) : 0;
        $map['is_del'] = 0;
        $map['is_audit'] = 1;
        $map['type'] = array('neq', 'weiba_post');
        $max_id && $map['feed_id'] = array('lt', $max_id);
        $feed_id = D('feed')->where($map)->field('feed_id')->select();
        $fids = getSubByKey($feed_id, 'feed_id');
        $feeds = $this->format_feed($fids);
        if (!$feeds) {
            return array(
            'status' => 0,
            'msg' => '暂无数据',
    ...
    public function weibo_detail()
    {
        ...
        $feed_info ['favoriteNum'] = model('Collection')->where(array(
                'source_id' => $feed_id,
    ...
    public function post_weibo($datas)
    {
        ...
        /* 标签（话题） */
        $topic_ids = t($this->data['topic_ids']);
        $topic_ids = array_filter(explode(',', $topic_ids));
        //判断系统标签是否存在，是否只有一个系统标签
        $topic_admin_count = model('FeedTopic')->where(array('create_uid' => 0, 'topic_id' => array('in', $topic_ids)))->count();

        if ($topic_admin_count < 1) {
            return array(
                'status' => 0,
                'msg' => '请至少选择一个系统标签',
            );
        } elseif ($topic_admin_count > 1) {
            return array(
                'status' => 0,
                'msg' => '最多只能选一个系统标签',
            );
        ...
        if ($feed_id && $feed_data) {
            ...
            // 添加与话题的关联
            $topic_data['feed_id'] = $feed_id;
            $topic_data['type'] = $data ['type'];
            foreach ($topic_ids as $k => &$v) {
                if ($v) {
                    $topic_data['topic_id'] = intval($v);
                    D('feed_topic_link')->add($topic_data);
                    //更新话题使用数
                    model('FeedTopic')->where(array('topic_id' => $topic_data['topic_id']))->setInc('count');
    ...
    public function user_collections()
    {
        ...
        $collection_count = D()->table('`'.C('DB_PREFIX').'feed` AS f LEFT JOIN `'.C('DB_PREFIX').'collection` AS c ON f.`feed_id` = c.`source_id`')->field('c.`source_id`,c.collection_id')->where($map)->count();
        ...
        if ($list && $collection_list) {
            return array('status' => 1, 'msg' => '获取成功', 'data' => array('collection_count' => $collection_count, 'list' => $collection_list));
        } else {
            return array('status' => 1, 'msg' => '暂无更多数据', 'data' => array('collection_count' => $collection_count, 'list' => array()));
        }
    }
     ...
     * @return array 文章评论列表
     */
    public function user_comments_to_me_information()
    {
        $where = " ( (app_uid = '{$this->mid}' or to_uid = '{$this->mid}') and uid != '{$this->mid}' )";
        $max_id = $this->max_id ? intval($this->max_id) : 0;
        $count = $this->count ? intval($this->count) : 10;
        $where .= ' AND is_del=0';
        $where .= ' AND app="Information"';
        model('UserData')->setKeyValue($this->mid, 'information_comment', 0);
        ...
        foreach ($list as $k => $v) {
            ...
            $information_info = D('information_list')->where(array('id' => $v['row_id']))->find();
            //文章信息处理
            $information_info['ctime'] = friendlyDate($information_info['ctime'], 'mohu');
            $information_info['rtime'] = friendlyDate($information_info['rtime'], 'mohu');
            $author_info = $this->get_user_info($information_info['author']);
            $information_info['author'] = $author_info['uname'];
            ...
            $information_info['image'] = $image ? $image : '';
            unset($information_info['content']);
            $comment ['content_info'] = $information_info;
            $comment_arr [] = $comment;
            unset($comment);
        }
    ...
    {
        if (in_array($this->data['type'], array('weiba_post', 'weibo'))) {
            $where = " ( (app_uid = '{$this->mid}' or to_uid = '{$this->mid}') and uid != '{$this->mid}' )";
            $max_id = $this->max_id ? intval($this->max_id) : 0;
            $count = $this->count ? intval($this->count) : 20;
            $where .= ' AND is_del=0';
            ...
            } elseif ($this->data['type'] == 'weibo') {
                $where .= ' AND app!="weiba" AND app != "Information"';
...
//              return array('status'=>1,'message'=>'获取成功','data'=>$comment_arr);
            return $comment_arr;
        } else {
            return array();
        }
    ...
    public function user_diggs_to_me()
    {
        if ($this->data['type'] == 'weibo') {
            ...
            return $digg_arr;
        } elseif ($this->data['type'] == 'weiba_post') {
            model('UserData')->setKeyValue($this->mid, 'weiba_digg', 0);
            $max_id = $this->max_id ? intval($this->max_id) : 0;
            $count = $this->count ? intval($this->count) : 20;
            $map ['f.post_uid'] = $this->mid;
            $map ['f.is_del'] = 0;
            !empty($max_id) && $map ['d.id'] = array(
                'lt',
                $max_id,
            ...
            $tablePrefix = C('DB_PREFIX');
            $list = D()->table("{$tablePrefix}weiba_post AS f RIGHT JOIN {$tablePrefix}weiba_post_digg AS d ON f.post_id = d.post_id ")->where($map)->order('d.id desc')->field('d.id as id,d.uid as uid,d.post_id as post_id,d.cTime as ctime')->limit($count)->findAll();
//           return $list;
            $digg_arr = array();
            foreach ($list as $k => $v) {
                $digg ['digg_id'] = $v ['id'];
                $digg ['feed_id'] = $v ['post_id'];
                $digg ['type'] = 'post';
                $digg ['content'] = '赞了这条微博';
                $digg ['ctime'] = friendlyDate($v['ctime'], 'mohu');
                $digg ['from'] = '来自网站';
                $digg ['user_info'] = $this->get_user_info($v['uid']);
                $digg ['content_info'] = D('weiba_post')->where(array('post_id' => $v['post_id']))->find();
                $digg ['content_info']['ctime'] = friendlyDate($digg['content_info']['post_time'], 'mohu');
                $author_info = $this->get_user_info($digg ['content_info']['post_uid']);
                $digg ['content_info']['author'] = $author_info['uname'];
                preg_match_all('/\<img(.*?)src\=\"(.*?)\"(.*?)\/?\>/is', $digg ['content_info']['content'], $image);
                $image = $image[2];
                if ($image && is_array($image) && count($image) >= 1) {
                    $image = $image[0];
                    if (!preg_match('/https?\:\/\//is', $image)) {
                ...
                $digg ['content_info']['image'] = $image ? $image : '';
                unset($digg ['content_info']['content']);
                unset($digg ['content_info']['post_time']);
                $digg_arr [] = $digg;
                unset($digg);
            }

            return $digg_arr;
        } elseif ($this->data['type'] == 'information') {
            model('UserData')->setKeyValue($this->mid, 'information_digg', 0);
            $max_id = $this->max_id ? intval($this->max_id) : 0;
            $count = $this->count ? intval($this->count) : 20;
            $map ['f.author'] = $this->mid;
            $map ['f.isDel'] = 0;
            !empty($max_id) && $map ['d.id'] = array(
                'lt',
                $max_id,
            ...
            $tablePrefix = C('DB_PREFIX');
            $list = D()->table("{$tablePrefix}information_list AS f RIGHT JOIN {$tablePrefix}information_digg AS d ON f.id = d.post_id ")->where($map)->order('d.id desc')->field('d.id as id,d.uid as uid,d.post_id as post_id,d.ctime as ctime')->limit($count)->findAll();
            $digg_arr = array();
            foreach ($list as $k => $v) {
                $digg ['digg_id'] = $v['id'];
                $digg ['post_id'] = $v['post_id'];
                $digg ['type'] = 'information';
                $digg ['content'] = '赞了这篇文章';
                $digg ['ctime'] = friendlyDate($v['ctime'], 'mohu');
                $digg ['from'] = '来自网站';
                $digg ['user_info'] = $this->get_user_info($v['uid']);
                $digg ['content_info'] = D('information_list')->where(array('id' => $v['post_id']))->find();

                $digg ['content_info']['ctime'] = friendlyDate($digg ['content_info']['ctime'], 'mohu');
                $digg ['content_info']['rtime'] = friendlyDate($digg ['content_info']['rtime'], 'mohu');
                $author_info = $this->get_user_info($digg ['content_info']['author']);
                $digg ['content_info']['author'] = $author_info['uname'];
                preg_match_all('/\<img(.*?)src\=\"(.*?)\"(.*?)\/?\>/is', $digg ['content_info']['content'], $image);
                $image = $image[2];
                if ($image && is_array($image) && count($image) >= 1) {
    ...
    public function get_feed_info($feed_id)
    {
        ...
        //关联标签（话题）
        $feed_info['topic_list'] = model('Feed')->getTopicIdByFeedId($data['feed_id']);
    ...
    public function all_topic()
    {
        $max_id = intval($this->data ['max_id']);
        $limit = intval($this->data['limit']);
        $select_topic_ids = t($this->data['topic_ids']);    //已选择的标签（话题）
        $name = t($this->data['name']);
        $order = 'topic_id desc';
        if ($name) {
            $map['topic_name'] = $map2['topic_name'] = array('like', '%'.$name.'%');
        } else {
            $order = 'count DESC';
        }
        if ($select_topic_ids) {
            $select_topic_ids = explode(',', $select_topic_ids);
            array_filter($select_topic_ids);
            if (model('FeedTopic')->where(array('create_uid' => 0, 'topic_id' => array('in', $select_topic_ids)))->count() > 0) {
                $map['create_uid'] = array('neq', 0);
            }
            $map['topic_id'] = array('not in', $select_topic_ids);
        }
        /*//暂时不用传推荐的话题
        if (empty($max_id)) {
            $map['create_uid'] ? ($map2['create_uid'] = $map['create_uid']) : '';
            $map['topic_id'] ? ($map2['topic_id'] = $map['topic_id']) : '';
            $map2 ['recommend'] = 1;
            $map2['lock'] = 0;
        ...
        $map['lock'] = 0;
        $res ['lists'] = (array) M('feed_topic')->where($map)->order($order)->limit($limit)->findAll();
        ...
        if (empty($res['lists'])) {
            return array('status' => 0, 'msg' => '暂无更多标签');
        }

        return array('status' => 1, 'msg' => '获取成功', 'data' => $res['lists']);
    }
    ...
    public function addTopic()
    {
        $topic_name = t($this->data['topic_name']);
        if (!$topic_name) {
            return array('status' => 0, 'msg' => '请输入标签名称');
        }
        $t = model('FeedTopic')->addTopic('#'.$topic_name.'#', false, null, $this->mid);
        if (!$t) {
            return array('status' => 0, 'msg' => '添加失败');
        }
        if (!$t[0]) {
            return array('status' => 0, 'msg' => '该标签已被添加');
        }

        return array('status' => 1, 'msg' => '添加成功', 'data' => array('create_uid' => $this->mid, 'topic_id' => $t[0], 'topic_name' => $topic_name));
    }
    ...
    {
        $limit = intval($this->data['limit']) ? intval($this->data['limit']) : 20;
        $max_id = intval($this->data ['max_id']);

        /* 获取推荐的热门微博（由后台配置） */
        $map2['f.is_recommend'] = 1;
        $map2['f.is_del'] = 0;
        $map2['f.is_audit'] = 1;
        $map2['f.type'] = 'postimage';
        $c_weibo = D()->table('`'.C('DB_PREFIX').'feed` AS f LEFT JOIN `'.C('DB_PREFIX').'feed_data` AS d ON f.`feed_id` = d.`feed_id`')
            ->field('d.`feed_id`,d.feed_data')
            ->where($map2)
            ->limit(3)
            ->order('d.feed_id DESC')
            ->findAll();

        foreach ($c_weibo as $k => &$v) {
            $v['feed_data'] = unserialize($v['feed_data']);
            $v['attach_id'] = getImageUrlByAttachId($v['feed_data']['attach_id'][0]);
            unset($v['feed_data']);
        }
        if ($c_weibo) {
            $commends['topic_id'] = -1;
            $commends['topic_name'] = '热门';
            $commends['count'] = 0;
            $commends['weibo'] = $c_weibo;
        }
        /*//现在是推荐的微博。若改成推荐话题，取消注释即可
        $map2 ['recommend'] = 1;
        $map2 ['lock'] = $map['lock'] = 0;
        $res ['commends'] = M('feed_topic')->where($map2)->order('count desc')->limit(3)->field('topic_id,topic_name,count')->findAll();
        $res ['commends'] = $res ['commends'] ? $res ['commends'] : [];*/
        /* 获取全部的热门的 */
        //只能查询话题里，微博有图的话题
        $ids = D()->table('`'.C('DB_PREFIX').'feed` AS f LEFT JOIN `'.C('DB_PREFIX').'feed_topic_link` AS t ON f.`feed_id` = t.`feed_id`')
            ->field('t.`topic_id`')
            ->where(array('f.type' => 'postimage'))
            ->group('t.`topic_id`')
            ->findAll();
        $ids = array_filter(getSubByKey($ids, 'topic_id'));
        $map['topic_id'] = array('in', $ids);

        if ($max_id) {
            $where ['topic_id'] = array('lt', $max_id);
            $map['_complex'] = $where;
        }
        /*//现在是推荐的微博。若改成推荐话题，取消注释即可
        if ($res ['commends']) {
            $map_ ['topic_id'] = array('not in', getSubByKey($res ['commends'], 'topic_id'));
            $map['_complex'] = $map_;
        }*/

        //根据条件进行查询
        $res['lists'] = M('feed_topic')->where($map)->order('count DESC')->field('topic_id,topic_name,count')->findPage($limit);
        $res['lists'] = $res['lists']['data'] ? $res['lists']['data'] : [];

        foreach ($res['lists'] as $kk => &$vv) {
            $vv && $vv['weibo'] = model('FeedTopic')->getFeedIdByTopicId($vv['topic_id'], 3);
        }
        $commends && array_unshift($res['lists'], $commends);
