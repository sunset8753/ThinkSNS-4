...
class FindPeopleApi extends Api
{
    ...
    public function search_user()
    {
        ...
        if ($key) {
            $map['uid'] = array(array('neq', 1), array('neq', $this->mid), 'and');
            $map['is_init'] = 1;
            $map['is_audit'] = 1;
            $map['is_active'] = 1;
            $map['is_del'] = 0;
            ...
            if ($uid_arr) {
                ...
                $user_list = (array) model('User')->where($map)->field('uid,uname,intro,department_id')->order('uid desc')->limit($count)->findAll();
            ...
            foreach ($user_list as $k => $v) {
                ...
                $user_list[$k]['department_list'] = getDepartmentById($v['department_id']);
                unset($user_list[$k]['department_id']);
            }
        } else { // 获取感兴趣的5个人
            $user = model('RelatedUser')->getRelatedUser($rus);
            $user_list = array();
            foreach ($user as $k => $v) {
                $photo = api('User')->user_photo($v['userInfo']['uid']);
                $user_list[$k]['uid'] = $v['userInfo']['uid'];
                $user_list[$k]['uname'] = $v['userInfo']['uname'];
                $user_list[$k]['avatar'] = $v['userInfo']['avatar_big'];
                $user_list[$k]['intro'] = $v['userInfo']['intro'] ? formatEmoji(false, $v['userInfo']['intro']) : $v['info']['msg'];
                $user_list[$k]['follow_status'] = model('Follow')->getFollowState($this->mid, $v['userInfo']['uid']);
                $user_list[$k]['department_list'] = getDepartmentById($v['userInfo']['department_id']);
                $user_list[$k]['gallery'] = '';
                if (count($photo) < 1) {
                    unset($user_list[$k]);
                } else {
                    $user_list[$k]['gallery'] = $photo;
                }
            }
        }
    ...
    {
        $list = model('Department')->getDepartment('1');
        //model('Department')->synchronizeDepart();
        return $this->showSuccess('获取成功', $list);
        /*$data = dingtalk_get_dept_list();
        foreach ($data->department as $key => $value) {
            $department_list[$key]['id'] = $value->id;
            $department_list[$key]['name'] = $value->name;
            $department_list[$key]['parentid'] = $value->parentid;
            if (!isset($value->parentid)) {
                unset($department_list[$key]);
            }
        }

        return array_values($department_list);*/
    }
     ...
     * @author zhangwei
     * @date   2016-10-11
     * @param  string $department_id 工作单位id
     * @return array
     */
    public function getDepartMentUser($department_id)
    {
        $dep_id = $department_id ? $department_id : $this->data['department_id'];

        $user = UserModel::where('department_id', 'like', '%'.$dep_id.'%')
            ->where(function ($query) {
                if ($this->data['max_id']) {
                    $query->where('uid', '<', $this->data['max_id']);
                }
            })->get();
        ...
        foreach ($user->toArray() as $k => $v) {
            ...
            $user_list[$k]['avatar'] = model('Avatar')->init($v['uid'])->getUserAvatar()['avatar_big'];
            $user_list[$k]['intro'] = $v['intro'];
            $user_list[$k]['follow_status'] = model('Follow')->getFollowState($this->mid, $v['uid']);
            $user_list[$k]['department_list'] = getDepartmentById($v['department_id']);
        }

        return $this->showSuccess('获取成功', $user_list);
    }

    public function getSysRelatedUser()
    {
        //$type = 5; //系统推荐用户
        //$rus = $this->data['rus'] ? $this->data['rus'] : 5;
        //$user = model('RelatedUser')->getRelatedUserByType($type, $rus); //注册配置推荐关注用户
        $user = model('UserOfficial')->getUserOfficialList(); //找人推荐用户
        if ($user['data']) {
            foreach ($user['data'] as $key => $value) {
                $user_info = api('User')->get_user_info($value['uid']);
                $user_list[$key]['uid'] = $value['uid'];
                $user_list[$key]['uname'] = $value['uname'];
                $user_list[$key]['avatar'] = $user_info['avatar']['avatar_big'];
                $user_list[$key]['intro'] = $value['intro'] ? $value['intro'] : '系统推荐用户';
                $user_list[$key]['follow_status'] = model('Follow')->getFollowState($this->mid, $value['uid']);
                $user_list[$key]['department_list'] = getDepartmentById($value['department_id']);
            }

            return $this->showSuccess('获取成功', $user_list);
        }

        return $this->showError('暂无推荐用户');
    }
}
