...
class WeibaApi extends Api
{
    ...
    public function deletePost()
    {
        ...
        if (!CheckWeibaPermission('', $post_id)) {
    ...
    public function post_detail()
    {
        ...
        $data['favoriteNum'] = D('weiba_favorite')->where('post_id='.$data['post_id'])->count();
        ...
        return $this->showSuccess('获取成功', $data);
    ...
    public function weiba_detail()
    {
        ...
        $new_weiba = $this->new_weibas($sfollow);
        $res['new_weiba'] = $new_weiba;

        return $this->showSuccess('获取成功', $res);
    }

    //关注工作室最新帖子
    protected function new_weibas($weiba_ids)
    {
        $where = array();
        $where['post_id'] = array('in', $weiba_ids);
        $where['is_del'] = 0;
        $newDatas = array();
        $new_weiba = D('weiba_post')->where($where)->order('post_time desc')->limit(20)->select();
        foreach ($new_weiba as $key => $val) {
            $userInfo = getUserInfo($val['post_uid']);
            $newDatas[$key]['post_id'] = $val['post_id'];
            $newDatas[$key]['time_format'] = friendlyDate($val['post_time'], 'mohu');
            $newDatas[$key]['post_time'] = $val['post_time'];
            $newDatas[$key]['title'] = $val['title'];
            $newDatas[$key]['reply_count'] = $val['reply_count'];
            $newDatas[$key]['read_count'] = $val['read_count'];
            $newDatas[$key]['user_info'] = $userInfo;
            $newDatas[$key]['user_info']['authorName'] = getUserName($val['post_uid']);
        }
    ...
    public function detail()
    {
        ...
        if (!$weiba_detail) {
            ...
            if ($follow) {
                ...
                $max_id = $this->max_id ? intval($this->max_id) : 0;
                $count = $this->count ? intval($this->count) : 20;
                !empty($max_id) && $maps['post_id'] = array('lt', $max_id);
                $data['follow'] = 1;
            } else {
    ...
    public function detail_digest()
    {
        ...
        return $this->showSuccess('获取成功', $list);
    }
    ...
    public function unFollowWeiba()
    {
        ...
        if (M('weiba_follow')->where($data)->find()) {
            ...
            if ($res) {
                ...
                // 添加积分
                model('Credit')->setUserCredit($data ['follower_uid'], 'unfollow_weiba');
                $nres ['status'] = 1;
                $nres ['msg'] = '取消关注成功';
    ...
    public function getUserFavorite()
    {
        ...
        $list = $this->_post_list($ids);

        /* # 获取帖子的收藏数 # */
        $collection_count = D('weiba_favorite')->where(array('uid' => $this->mid))->count();

        if (!$list) {
            return array('status' => 0, 'msg' => '暂无更多数据');
        }
        $weibaData = array('collection_count' => $collection_count, 'data' => $list);
     ...
     *        	integer limit 获取圈子条数
     */
    public function _weiba_all($limit = 8, $width = 200, $height = 200)
    {
        $this->user_id = empty($this->user_id) ? $this->mid : $this->user_id;
        $map ['status'] = 1;
        $map ['is_del'] = 0;
     ...
     * @return array 评论列表
     */
    public function weiba_comments($post_id, $count)
    {
        $postid = $post_id ? $post_id : $this->data['post_id'];
        $post_id = M('weiba_post')->where(array('post_id' => $postid))->getField('post_id');
        if (!$post_id) {
            return;
        }
        $comment_list = array();
        $where = 'is_del=0 and post_id='.$post_id;
        if (!$count) {
            $count = $this->count;
        ...
        $floor = '';
        if ($this->max_id) {
            $floor = M('weiba_reply')->where(array('is_del' => 0, 'post_id' => $post_id, 'reply_id' => array('gt', $this->max_id)))->count();
        } else {
            $floor = M('weiba_reply')->where(array('is_del' => 0, 'post_id' => $post_id))->count();
        }
    ...
    private function get_user_info($uid)
    {
        ...
        $user_info['avatar_middle'] = $user_info_whole['avatar']['avatar_middle'];
    ...
    public function add_post_digg()
    {
        ...
        if ($res) {
            ...
            $result ['msg'] = '操作成功';
            //添加帖子点赞信息
            $pubId = M('weiba_post')->where(array('post_id' => $maps['post_id']))->find();
            model('UserData')->setUid($pubId['post_uid'])->updateKey('weiba_digg', 1);
    ...
    public function myPosts()
    {
        ...
        if ($uid == $this->mid || !$uid) {
            ...
            $wmap['w.is_del'] = 0;
            $wmap ['w.status'] = 1;
            $wmap ['f.follower_uid'] = $this->mid;
            $collection_weiba = D()->table('`'.C('DB_PREFIX').'weiba` AS w LEFT JOIN `'.C('DB_PREFIX').'weiba_follow` AS f ON w.`weiba_id` = f.`weiba_id`')
                ->field('w.weiba_id,w.weiba_name,w.logo')
                ->where($wmap)
                ->order('w.weiba_id DESC')
                ->findAll();
            foreach ($collection_weiba as $k => &$v) {
                $v && $v['logo'] = getImageUrlByAttachId($v['logo']);
            }
        }
        $collection_weiba = $collection_weiba ? $collection_weiba : array();
        // 推荐帖子
        $map ['post_uid'] = $uid ? $uid : $this->mid;
        $map ['is_del'] = 0;
        empty($this->max_id) || $map ['post_id'] = array(
            'lt',
            $this->max_id,
        ...
        $list = $this->_post_list($list);
        if (!$list) {
            return array('status' => 0, 'msg' => '暂无更多数据', 'data' => array());
        }
        foreach ($list as $k => &$v) {
            $v['weiba'] = D('weiba')->where(array('weiba_id' => $v['weiba_id']))->field('weiba_id,weiba_name,uid,logo,follower_count,thread_count')->find();
            $v['weiba']['logo'] = getImageUrlByAttachId($v['weiba']['logo']);
        }

        return array('status' => 1, 'msg' => '获取成功', 'collection_weiba' => $collection_weiba, 'data' => $list);
    }
}
